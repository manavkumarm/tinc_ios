...
static bool setup_device(void) {
    ...
    // Set timeout on Write socket so write operations won't timeout
    struct timeval write_sock_timeout;
    write_sock_timeout.tv_sec = 10;
    setsockopt(write_fd, SOL_SOCKET, SO_RCVTIMEO, &write_sock_timeout,
               sizeof write_sock_timeout);
    setsockopt(write_fd, SOL_SOCKET, SO_SNDTIMEO, &write_sock_timeout,
               sizeof write_sock_timeout);
    ...
    logger(LOG_DEBUG, "Name struct contents: zero = %c, pid = %d, usecs = %d",
           name.zero, name.pid, name.usecs);
	data_sun.sun_family = AF_UNIX;
	//memcpy(&data_sun.sun_path, &name, sizeof name);
    /********* BEGIN CHANGE: Manav Kumar Mehta, Jun 2017 *********/
    // Temporary patch: hardcoding the file name for the Data socket and setting it in
    // the same path as the Listen socket
    // The getpid() and gettimeofday() functions above do not seem to work on iOS
    // This patch is to set the file name till the above issue can be resolved
    const char tempname[] = "tincdwrite.socket";
    char *device_fname;
    char data_socket_path[200];
    if (NULL != strrchr(device, '/')) {
        strncpy(data_socket_path, device, strlen(device) + 1);
        device_fname = strrchr(data_socket_path, '/') + 1;
        strncpy(device_fname, tempname, strlen(tempname) + 1);
    }
    memcpy(&data_sun.sun_path,
           &data_socket_path,
           sizeof(char) * (strlen(data_socket_path) + 1));
    ifdebug(STATUS) {
        logger(LOG_DEBUG, "Data socket path: %s", data_sun.sun_path);
    }
    unlink(data_sun.sun_path);
    /********* END CHANGE: Manav Kumar Mehta, Jun 2017 *********/
    ...
    /********* BEGIN CHANGE: Manav Kumar Mehta, Jun 2017 *********/
    // Temporary code to unlink the Request socket path before binding to it
    //
    // TO DO: Remove this code after implementing the function to stop Tinc when it's running
    // as a library
    unlink(listen_sun.sun_path);
    /********* END CHANGE: Manav Kumar Mehta, Jun 2017 *********/
...
static bool read_packet(vpn_packet_t *packet) {
	...
	switch(state) {
		case 0: {
            ...
            /********* BEGIN CHANGE: Manav Kumar Mehta, Jun 2017 *********/
            // Set timeout on Request socket so read and write operations won't timeout
            struct timeval req_sock_timeout;
            req_sock_timeout.tv_sec = 10;
            setsockopt(request_fd, SOL_SOCKET, SO_RCVTIMEO, &req_sock_timeout,
                       sizeof req_sock_timeout);
            setsockopt(request_fd, SOL_SOCKET, SO_SNDTIMEO, &req_sock_timeout,
                       sizeof req_sock_timeout);
            /********* END CHANGE: Manav Kumar Mehta, Jun 2017 *********/
		...
		case 1: {
			if((lenin = read(request_fd, &request, sizeof request)) != sizeof request) {
                logger(LOG_DEBUG, "Bytes read = %d, expected = %lu", lenin, sizeof request);
            ...
            /********* BEGIN CHANGE: Manav Kumar Mehta, Jun 2017 *********/
            // Temporary patch as workaround to the problem of correctly passing the socket path
            // string between Swift and C
            if (AF_UNIX != request.sock.sun_family)
                request.sock.sun_family = AF_UNIX;
            const char writesockname[] = "tincdread.socket";
            char *device_fname;
            char write_socket_path[200];
            if (NULL != strrchr(device, '/')) {
                strncpy(write_socket_path, device, strlen(device) + 1);
                device_fname = strrchr(write_socket_path, '/') + 1;
                strncpy(device_fname, writesockname, strlen(writesockname) + 1);
            }
            memcpy(&request.sock.sun_path,
                   &write_socket_path,
                   sizeof(char) * (strlen(write_socket_path) + 1));
            request.sock.sun_len = 0;
            ifdebug(STATUS) {
                logger(LOG_DEBUG, "Write UML socket - family: %s, len: %d, path: %s",
                       (AF_UNIX == request.sock.sun_family) ? "AF_UNIX" : "Other",
                       request.sock.sun_len,
                       request.sock.sun_path);
            }
            /********* END CHANGE: Manav Kumar Mehta, Jun 2017 *********/
