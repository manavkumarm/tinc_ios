...
/* Utility functions for reading hardcoded public and private keys */
RSA *xPEM_read_RSAPublicKey_from_str(const char *key, RSA **rsa) {
  RSA *retrsa = (rsa) ? *rsa : NULL;
...
RSA *xPEM_read_RSAPrivateKey_from_str(const char *key, RSA **rsa) {
  RSA *retrsa = (rsa) ? *rsa : NULL;

  BIO *keybio = BIO_new_mem_buf(key, strlen(key));
  if (!keybio) {
  	logger(LOG_ERR, "Could not read private key string into struct");
  	return NULL;
  }

  if (!(retrsa = PEM_read_bio_RSAPrivateKey(keybio, rsa, NULL, NULL))) {
    logger(LOG_ERR, "Failed to create RSA struct for private key");
  }
...
*/
	...
	/* Load a hardcoded RSA key into the RSA struct */
	c->rsa_key = xPEM_read_RSAPublicKey_from_str(get_hardcoded_public_key(c->name), &c->rsa_key);
    /********* END CHANGE: Manav Kumar Mehta, Jun 2017 *********/
...
*/
	...
	/* Read hardcoded private key from string */
	logger(LOG_DEBUG, "Loading hardcoded private key...");
	myself->connection->rsa_key = xPEM_read_RSAPrivateKey_from_str(server_private_key, NULL);

	if(!myself->connection->rsa_key) {
		/* logger(LOG_ERR, "Reading RSA private key file `%s' failed: %s",
			   fname, strerror(errno)); */
        logger(LOG_ERR, "Reading RSA private key failed: %s",
               strerror(errno));
		//free(fname);
		return false;
	}
...
/*		read_config_options(config_tree, ent->d_name);
		...
		if (!add_hardcoded_connection_config(config_tree, ent->d_name, fname)) {
			logger(LOG_ERR, "Could not locate connection config for %s", ent->d_name);
		}
        /********* END CHANGE: Manav Kumar Mehta, Jun 2017 *********/
		free(fname);
...
/*	read_config_options(config_tree, name);
	read_config_file(config_tree, fname); */
	if (!add_hardcoded_connection_config(config_tree, name, fname)) {
		logger(LOG_ERR, "Could not locate connection config for %s", name);
	}
    /********* END CHANGE: Manav Kumar Mehta, Jun 2017 *********/
	free(fname);
...
#endif
    /********* BEGIN CHANGE: Manav Kumar Mehta, Jun 2017 *********/
    // Temporarily comment out tinc-up script execution for iOS
    // It causes a runtime failure on iOS
    //
    // TO DO: Un-comment this line and wrap it inside a pre-processor macro
    // so that the same code can eventually be compiled for either iOS or other
    // platforms using a -D compiler switch
...
void close_network_connections(void) {
    ...
    /********* BEGIN CHANGE: Manav Kumar Mehta, Jun 2017 *********/
    // Temporarily comment out tinc-down script execution for iOS Proof of Concept
    // It causes a runtime failure on iOS
    //
    // TO DO: Un-comment this line and wrap it inside a pre-processor macro
    // so that the same code can eventually be compiled for either iOS or other
