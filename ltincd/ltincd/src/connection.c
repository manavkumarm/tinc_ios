...
void dump_connections(void) {
	...
	for(node = connection_tree->head; node; node = node->next) {
		...
		/* Dump configuration for connection, which was set from reading hosts file or hard-coded */
		char *hostname = malloc(500);
		xasprintf(&hostname, "HOST %s", c->name);
		dump_config(c->config_tree, hostname);
		free(hostname);
