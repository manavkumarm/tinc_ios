...
// Header files required for utility functions for printing out the hardcoded configuration for debugging
#include <stdio.h>
...
/********* BEGIN CHANGE: Manav Kumar Mehta, Jun 2017 *********/
// Utility function for printing out the hardcoded configuration for debugging
void dump_config(avl_tree_t *ctree, const char *cname) {
    avl_node_t *n;
    config_t *c;

    logger(LOG_DEBUG, "**** %s:", cname);
    for (n = ctree->head; NULL != n; n = n->next) {
      c = (config_t *)n->data;
      logger(LOG_DEBUG, "%s = %s", c->variable, c->value);
    } /* for (...config_tree...) */
